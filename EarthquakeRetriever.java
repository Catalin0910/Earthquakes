...
public class EarthquakeRetriever {
    public ArrayList<Earthquake> getEarthquakes(String countryCode, int days) throws IOException {
        ...
        for (int i = 0; i < features.length(); i++) {
            ...
            String country =  properties.getString("title");
            double magnitude = properties.getDouble("mag");
            String location = properties.getString("place");
            long timeInMilliseconds = properties.getLong("time");
            LocalDateTime timestamp = LocalDateTime.ofEpochSecond(timeInMilliseconds / 1000,0, ZoneOffset.UTC);
            earthquakes.add(new Earthquake(country,magnitude, location, timestamp));
        }
        return earthquakes;
    }
}
